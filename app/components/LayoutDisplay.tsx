...
const Layout = React.forwardRef<LayoutDisplayRef, LayoutProps>(({ config, children, theme, buttonIcon, hoverDelayMs = 300, nextScreenTemplate, outlineTarget, onConfigChange, onHoverInfoChange, onActionEvent }: LayoutProps, ref) => {
  ...
  const handleCloseScreen = React.useCallback(
    (path: string) => {
      if (path === 'root' || !getNodeAtPath(committedConfig, path)) {
        layoutLog('action', 'handleCloseScreen early return', { path, pathExists: !!getNodeAtPath(committedConfig, path) });
        return;
      }
      ...
      layoutLog('action', 'closeScreen', { path });
      ...
      const cleanup = scheduleOnWeb(() => {
        ...
        layoutLog('action', 'handleCloseScreen cleanup done', { path });
      }, durationMs + WEB_STAGE_DELAY_MS * 2);
  ...
  React.useImperativeHandle(ref, () => ({
    ...
    showPreview: handleActionHoverIn,
    hidePreview: handleActionHoverOut,
    executeAction: handleActionPress,
  }), [handleCloseScreen, handleActionHoverIn, handleActionHoverOut, handleActionPress]);
...
export const LayoutScreen = (_props: ScreenSlotProps) => null;
  ...
  const inner = (
    <Pressable
      ...
      onPressIn={() => {
        ...
        onActionEvent?.({ type: 'press-start', target: 'button', id: action.key, x: 0, y: 0, action });
      }}
      onPressOut={() => {
        if (!isInteractive) {
          return;
        }
        onActionEvent?.({ type: 'press-end', target: 'button', id: action.key, x: 0, y: 0, action });
      }}
      onPress={() => {
        if (!isInteractive) {
          return;
        }
        // Execution is handled by Layout.tsx on press-end
      }}
      style={[
        baseStyle,
...
const SplitGap = ({
  ...
  const attachDragListeners = React.useCallback(
    ...
    [isInteractive, path, index, onGapDragStart, onGapDragMove, onGapDragEnd, action, onActionEvent],
  );
